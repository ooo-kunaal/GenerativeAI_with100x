...
4. Classify (Gemini LLM, batched 40 at a time)
   ...
   ▼
5. Benchmark each item (rule-based KB lookup)
...
6. Flag (rule engine)
   ...
   ▼
7. AI reasonableness check (Gemini LLM)
...
Policy PDF upload + Sum Insured / Top-up / Add-ons
   │
   ▼
1. Ingest policy PDF (same OCR pipeline as bills)
   │
   ▼
2. Score paragraphs by policy keywords
   ...
   ▼
3. Extract policy parameters (Gemini LLM)
   ...
   │    exclusions
   ▼
4. Apply user-declared add-on overrides
   │  → room-rent waiver, no co-pay, OPD, maternity riders
   ▼
5. Claim engine — apply deductions in order:
   │
